...
def normalize(dataSet1):
	dataSet = dataSet1[:,-2:-4:-1]
	n = shape(dataSet)[1]
	# centroids = mat(zeros((k,n)))
	# maxNumber = []
	# minNumber = []
	# for j in range(n):
		# minNumber[j] = min(dataSet[:,j])
		# maxNumber[j] = max(dataSet[:,j])
		# minJ = min(dataSet[:,j])
		# maxJ = max(dataSet[:,j])
		# dataSet[:,j] = (dataSet[:,j]-minJ)/(maxJ-minJ)+0.0001

	# below new normalize method. set norDist 970Km, set notesnumber 9;
	dataSet[:,0] = dataSet[:,0]/970
	dataSet[:,1] = dataSet[:,1]/9
	return dataSet
...
def kMeans(dataSet, k, distMeas=distEclud, createCent=randCent):
    ...
    while clusterChanged:
        ...
        for i in range(m):#for each data point assign it to the closest centroid
            minDist = inf; minIndex = -1   #inf is infinity - a value that is greater than any other value. -inf is therefore smaller than any other value.
            for j in range(k):
